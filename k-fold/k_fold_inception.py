...
all_covid_files = glob.glob('train/covid/*')
all_normal_files = glob.glob('train/normal/*')
all_pneumo_files = glob.glob('train/pneumonia/*')
...
train_idx_total = p[:len(df_train)]
...
# Begin the k-fold cross validation

for train_index in kf.split(train_idx_total):
    fold_no += 1
    # print('train count: %s' % (len(train_index)))
    print(train_index[0], train_index[1])
    print(len(train_index[0]), len(train_index[1]))
    k = 0
    hist = {'pneumonia':0, 'covid':0, 'normal': 0 }
    ...
    #         break
    #     print(file, " ", label)
    train_idx = train_index[0]
    test_idx = train_index[1]
    ...
    hist_json_file = 'history_CXR_Covid-19_{}e_{}_{}_{}_fold_{}.json'.format(EPOCHS, MODEL_NAME,IMG_SIZE,OUTPUT_LAYERS,fold_no)
    with open(hist_json_file, mode='w') as f:
        hist_df.to_json(f)

    # download the model in computer for later use
    model.save('CXR_Covid-19_{}e_{}_{}_{}_fold_{}.h5'.format(EPOCHS, MODEL_NAME,IMG_SIZE,OUTPUT_LAYERS,fold_no))



    from tensorflow import keras
    model = keras.models.load_model('CXR_Covid-19_{}e_{}_{}_{}_fold_{}.h5'.format(EPOCHS, MODEL_NAME,IMG_SIZE,OUTPUT_LAYERS,fold_no), custom_objects={'specificity': specificity, 'sensitivity': sensitivity})
    ...
    with open('y_test_list_{}e_{}_{}_{}_fold_{}.pkl'.format(EPOCHS, MODEL_NAME,IMG_SIZE,OUTPUT_LAYERS,fold_no), 'wb') as f:
        pickle.dump(y_test_list, f)
    with open('y_pred_list_{}e_{}_{}_{}_fold_{}.pkl'.format(EPOCHS, MODEL_NAME,IMG_SIZE,OUTPUT_LAYERS,fold_no), 'wb') as f:
        pickle.dump(y_pred_list, f)

    with open('y_test_list_{}e_{}_{}_{}_fold_{}.pkl'.format(EPOCHS, MODEL_NAME,IMG_SIZE,OUTPUT_LAYERS,fold_no), 'rb') as f:
        y_test_list = pickle.load(f)
    with open('y_pred_list_{}e_{}_{}_{}_fold_{}.pkl'.format(EPOCHS, MODEL_NAME,IMG_SIZE,OUTPUT_LAYERS,fold_no), 'rb') as f:
